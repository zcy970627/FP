...
from nondominated import NonDominatedSorting
# Usage Example
...
def get_output_filename(args):
    if args.output != None:
        ...
        out_file_name = 'Hill_opt_'+prefix+'_Step'+img_step+'_'+'Bitwidth'+str(pipeline_bitwidth)+ objectives+'_Gen'+str(n_generations) +'_Section'+str(uni_lut_max_sections)+'_nMu'+str(n_mu)+'_pCross'+str(p_cross)+'_limSum16.pickle'
...
def hillclimbing(start_pt, n_iterations, fitness1, fitness2, fitness3, k):   
    ...
    for i in range(1,n_iterations,1): #number of iterations
        ...
        for j in range(len(P)): #make comparing with all elements in P
            fitness_P_j = P[j][1] #get fitness value in P

            if fitness_P_j[0]>=y1 and fitness_P_j[1]>=y2 and fitness_P_j[2]>=y3: # absolutely better
                stagnation_counter = 0
                P[j] = candidate_x_fit #replace
                start_pt = candidate_x

            elif y1<fitness_P_j[0] or y2<fitness_P_j[1] or y3<fitness_P_j[2]:  # pareto front
        ...
        if i%100== 0:
            ...
            # pareto_front = NonDominatedSorting.find_non_dominated(np.array(pareto_front))
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        ax.set_ylabel('MaxDE')
        ax.set_xlabel('Power')
        plt.savefig('/work/publications/norcas2020/experiments/results/1/3dPareto'+str(i+1)+'.png')
        plt.show()

        for k in range(0, len(pareto_front)):
            plt.plot(pareto_front[k][0],pareto_front[k][1],"+r")
        plt.savefig('/work/publications/norcas2020/experiments/results/1/2dPareto'+str(i+1)+'.png')
        plt.show()

        f = open('/work/publications/norcas2020/experiments/results/1/hv_list'+str(i+1)+'.pkl','wb')
        pickle.dump(hv_list,f)
        f.close()

        f = open('/work/publications/norcas2020/experiments/results/1/P'+str(i+1)+'.pkl','wb')
        pickle.dump(P,f)
        f.close()

        f = open('/work/publications/norcas2020/experiments/results/1/ParetoFront'+str(i+1)+'.pkl','wb')
        ...
        plt.savefig('/work/publications/norcas2020/experiments/results/1/Convergence'+str(i+1)+'.png')
        plt.show()

    file = open('/work/publications/norcas2020/experiments/results/1/HV1.txt','w')
    for hv in hv_all:
        file.write(str(hv))
        file.write('\n')
    file.write('Average Hypervolume of Neighbor Function 1 after 40000 Generations is')
    file.write('\b')
