...
def FindNeighbor(start_pt):
    ...
    if pos == 1:
        ...
        pos2 = random.choice([1,2,3])
        
        if pos2 == 1:
            start_pt[1][1][1][0] = random.randint(0,13)
            start_pt[1][1][1][1] = random.randint(0,13)
            start_pt[1][1][1][2] = random.randint(0,13)
            
            mx = np.max(start_pt[1][1][1])
            start_pt[1][1][2] = random.randint(0,mx)
            
            start_pt[1][1][3][0] = random.randint(1,2)
            start_pt[1][1][3][1] = random.randint(1,2)
            
            for i in range(2):
                if start_pt[1][1][3][i] == 2:
                    start_pt[1][1][4][i] = random.randint(0,1)
                else:
                    start_pt[1][1][4][i] = 0
            
            start_pt[1][1][-1][0] = random.randint(0,12+mx)
            start_pt[1][1][-1][1] = random.randint(0,12+mx)
            
        elif pos2 == 2:
            start_pt[1][2][1][0] = random.randint(0,13)
            start_pt[1][2][1][1] = random.randint(0,13)
            start_pt[1][2][1][2] = random.randint(0,13)
            
            mx = np.max(start_pt[1][2][1])
            start_pt[1][2][2] = random.randint(0,mx)
            
            start_pt[1][2][3][0] = random.randint(1,2)
            start_pt[1][2][3][1] = random.randint(1,2)
            
            for i in range(2):
                if start_pt[1][2][3][i] == 2:
                    start_pt[1][2][4][i] = random.randint(0,1)
                else:
                    start_pt[1][2][4][i] = 0
            
            start_pt[1][2][-1][0] = random.randint(0,12+mx)
            start_pt[1][2][-1][1] = random.randint(0,12+mx)
            
        else:
            start_pt[1][3][1][0] = random.randint(0,13)
            start_pt[1][3][1][1] = random.randint(0,13)
            start_pt[1][3][1][2] = random.randint(0,13)
            
            mx = np.max(start_pt[1][3][1])
            start_pt[1][3][2] = random.randint(0,mx)
            
            start_pt[1][3][3][0] = random.randint(1,2)
            start_pt[1][3][3][1] = random.randint(1,2)
            
            for i in range(2):
                if start_pt[1][3][3][i] == 2:
                    start_pt[1][3][4][i] = random.randint(0,1)
                else:
                    start_pt[1][3][4][i] = 0
            
            start_pt[1][3][-1][0] = random.randint(0,12+mx)
            start_pt[1][3][-1][1] = random.randint(0,12+mx)
        
        
    else:
        pos3 = random.choice([1,2,3])
        if pos3 == 1:
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        ax.set_ylabel('MaxDE')
        ax.set_xlabel('Power')
        plt.savefig('/work/publications/norcas2020/experiments/results/6/3dPareto'+str(i+1)+'.png')
        plt.show()

        for k in range(0, len(pareto_front)):
            plt.plot(pareto_front[k][0],pareto_front[k][1],"+r")
        plt.savefig('/work/publications/norcas2020/experiments/results/6/2dPareto'+str(i+1)+'.png')
        plt.show()

        f = open('/work/publications/norcas2020/experiments/results/6/hv_list'+str(i+1)+'.pkl','wb')
        pickle.dump(hv_list,f)
        f.close()

        f = open('/work/publications/norcas2020/experiments/results/6/P'+str(i+1)+'.pkl','wb')
        pickle.dump(P,f)
        f.close()

        f = open('/work/publications/norcas2020/experiments/results/6/ParetoFront'+str(i+1)+'.pkl','wb')
        ...
        plt.savefig('/work/publications/norcas2020/experiments/results/6/Convergence'+str(i+1)+'.png')
        plt.show()


    file = open('/work/publications/norcas2020/experiments/results/6/HV6.txt','w')
    for hv in hv_all:
        file.write(str(hv))
        file.write('\n')
    file.write('Average Hypervolume of Neighbor Function 6 after 40000 Generations is')
    file.write('\b')
