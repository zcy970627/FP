...
df = open('/work/publications/norcas2020/experiments/results/2/ParetoFront1.pkl','rb')
data = pickle.load(df)
df.close()

for k in range(0, len(data)):
    plt.plot(data[k][0],data[k][1],".k")
plt.xlabel('Power[mW]')
plt.ylabel('Maximum DE')
plt.savefig('/work/publications/norcas2020/experiments/results/2dProjection.eps')
plt.show()

# pareto_front1
I = efficient_non_dominated_sort(np.array(data))
# print(I)
...
pareto = pareto[:12]
# print(pareto)
b = np.unique([j for i in pareto for j in i])
# print(b)
# print(len(b))
...
for k in range(0, len(T)):
    plt.plot(T[k][0],T[k][1],".k")
plt.xlabel('Power[mW]')
plt.ylabel('Maximum DE')
plt.savefig('/work/publications/norcas2020/experiments/results/2dProjection_1452.eps')
plt.show()
